...
export interface ResourceLimits {
  /** Number of CPU cores allocated (e.g. 1, 2, 0.5). Shared hard rate-cap semantics with cpuQuota (RFC 0007); cpuQuota takes precedence when set. Throttles, never kills; enforcement reported via cpuQuotaLimits (native Linux and Windows true where their probes pass; Docker pending). */
  cpu?: number;
  /** CPU core allocation quota in cores, fractional allowed (RFC 0007; native Linux and Windows enforce where their probes pass). Takes precedence over cpu; values at or below zero mean unset. */
  cpuQuota?: number;
...
export interface SandboxOptions {
  ...
  cpu?: number;
  /** CPU core allocation quota in cores, fractional allowed (RFC 0007; native Linux and Windows enforce where their probes pass; values at or below zero mean unset). */
  cpuQuota?: number;
  /** CPU time budget in milliseconds (e.g. 2000). Enforced across the process group by the native backend; enforced per process via RLIMIT_CPU (one-second granularity) by the docker backend. */
  cpuTimeLimit?: number;
...
export interface ExecOptions {
  ...
  cpuTimeLimit?: number;
  /** CPU core allocation quota in cores for this execution (RFC 0007; native Linux and Windows enforce where their probes pass). Overrides the sandbox quota for this execution only. */
  cpuQuota?: number;
