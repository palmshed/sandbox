...
- **Discovery and probe**: at `init()`, locate a writable parent in order: the `PALMSHED_CGROUP_PARENT` environment override (operator escape hatch), the systemd user delegation path, then a direct child of the cgroup root. The probe creates, writes, and removes a test directory; success reports the capability `true`, anything else `false`. cgroup v1 hierarchies are out of scope and report `false`.
...
- **Residuals to declare**: the spawn-to-move window (milliseconds of unthrottled CPU, negligible against any real quota); rmdir best effort when threads linger; quotas above host capacity never bind (Q6).
