...
- **Residuals to declare**: the spawn-to-move window (milliseconds of unthrottled CPU, negligible against any real quota); rmdir best effort when threads linger; quotas above host capacity never bind (Q6). Setup relocates the SDK host itself into `<parent>/palmshed-self` before enabling `+cpu` in the parent: enabling controllers in a member-holding cgroup violates the no-internal-process constraint (moves into its children are then refused), so the parent must be member-free with respect to us first; the host stays relocated and unthrottled, which is harmless, and everything cleans up with the scope. The probe demonstrates a real live-PID move per candidate because writability alone never proves a move is possible (a bogus PID fails before the migration checks). For deterministic results, run quota workloads under a private scope (`systemd-run --user --scope`); shared login sessions may honestly report `false`.
...
- **Wiring**: the SDK is dependency-free TypeScript, so job management goes through PowerShell with inline C# P/Invoke (create plus set-rate at sandbox create; set-rate around per-exec overrides under the same mutex discipline; query-rate for test readback). One call per sandbox plus one per override keeps the overhead off the unenforced path.
- **Assignment edge**: a process can belong to only one job; if assignment fails (host already in a job, Remote Desktop DFSS environments where rate control is unavailable), sandbox creation with a quota fails honestly with `EXEC_FAILED` rather than running uncapped. Without a quota the backend behaves exactly as today.
