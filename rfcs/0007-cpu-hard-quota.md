...
- **Residuals to declare**: the spawn-to-move window (milliseconds of unthrottled CPU, negligible against any real quota); rmdir best effort when threads linger; quotas above host capacity never bind (Q6). Sandbox groups live under a process-free distributor child that carries the `+cpu` enablement downward: enabling controllers in a member-holding cgroup violates the no-internal-process constraint (moves into its children are then refused), and writability alone never proves a live move is possible, so the probe demonstrates a real move per candidate.
