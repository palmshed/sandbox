...
> **Status**: Complete for the **v1.0 scope** (2026-08-08), with RFC 0006
> OS-level filesystem isolation shipped in **v1.1.0** (2026-09-06) as an
> optional, capability-reported extension. Residual items:
...
> - **Hard `cpuQuota`**: designed in RFC 0007 (throttle, never kill), specified under Unreleased target 1.2.0, reported via the `cpuQuotaLimits` capability. Native Linux implemented and validated green on real Linux (quota suites need a private scope); native Windows implemented and validated green on real Windows (Job Objects with per-exec self-assign prefix); Docker implemented and validated green on CI Ubuntu (daemon API probe)
...
- [x] **Filesystem Hardening**: VFS boundary v0.1.2 (traversal / absolute-path / symlink-escape rejection, `workDir` containment, exec-path disk quota with rollback); OS-level isolation implemented post-v1.0 as an optional capability (RFC 0006, issue `#3`)
