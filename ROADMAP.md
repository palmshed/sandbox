...
> OS-level filesystem isolation implemented post-v1.0 (2026-08-10) as an
> optional, capability-reported extension (no version bump). Residual items:
> - **OS-level filesystem isolation** for the Native backend (Landlock confinement runner + runtime allowlist): **implemented** (RFC 0006, issue `#3`); reported at runtime as `osFilesystemIsolation` (`supported` | `unsupported` | `unknown`). Linux + Landlock ABI >= 2 + unprivileged user namespaces report `supported` after a real confined self-test; macOS remains `unknown` (Seatbelt FS profile pending validation); Windows reports `unsupported`
