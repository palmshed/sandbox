...
> - **Hard `cpuQuota`**: designed in RFC 0007 (throttle, never kill), specified under Unreleased target 1.2.0, reported via the `cpuQuotaLimits` capability. Native Linux implemented and validated green on real Linux (quota suites need a private scope); Windows and Docker implementations pending per the RFC sequence
