...
│   ├── scenarios/         # ai-agent, ci-runner, code-evaluator, concurrency, recovery, crash-recovery, resilience, artifacts, os-filesystem-isolation
...
`cpuLimits` reflects CPU **time** budget enforcement (Linux/macOS: supported via process-group sampling; Windows: best-effort PowerShell `Get-CimInstance` process-tree polling). The hard core quota (`cpuQuota`) is designed in RFC 0007 and reported separately via the `cpuQuotaLimits` boolean (throttle, never kill): `true` on Linux where cgroup delegation probes pass (validated green on real Linux; quota suites need a private scope since extra scope members make the probe honestly report `false`), `true` on Windows where Job Object rate control probes pass (validated green on real Windows; per-exec self-assign prefix joins deterministically), `false` for macOS only (no unprivileged hard-cap API); Docker reports `true` where the daemon API supports container updates (validated green on CI Ubuntu).
...
`osFilesystemIsolation` (RFC 0006) is a tri-state string, NOT a boolean: `supported` | `unsupported` | `unknown`. It is probed at `init()`:
- **Linux**: `supported` only after the embedded Landlock confinement runner (source: `scripts/probes/landlock-run.c`, embedded as `sdk/typescript/src/osfs/landlockRunnerSource.ts`) compiles, the runtime allowlist derives, and a real confined self-test passes through the exact `unshare --user --map-root-user -- <runner> <ws> <allowlist> -- <cmd>` chain the backend uses. `unsupported` when Landlock ABI < 2 or unavailable; `unknown` when the probe/setup failed (no compiler, `ldd` failure, or user namespaces restricted).
- **macOS**: `unknown` (Seatbelt filesystem profile is pending validation).
- **Windows / other**: `unsupported`.
- **Opt-out**: `Sandbox.create({ osFilesystemIsolation: false })` disables confinement for that sandbox even when supported (documented opt-out; used by `ci-runner`/`ai-agent` production scenarios because `npm` is outside the minimal runtime allowlist). Capability is still reported truthfully.
- The embedded C source is regenerated by `scripts/gen-osfs-source.mjs` (`--check` guards drift). Never edit the embedded string by hand; edit `scripts/probes/landlock-run.c` and regenerate.

The **Docker backend** reports only capabilities backed by implementation and integration tests: `filesystem: true`, `streaming: true`, `remoteExecution: false`, and `osFilesystemIsolation: 'unsupported'`. `cpuLimits`, `memoryLimits`, and `networkIsolation` are `true` since backend-parity work item `#4` (CPU time budgets via RLIMIT_CPU with ERR_CPU_EXCEEDED, per-execution memory overrides via container-wide `docker update` with OOMKilled attribution and ERR_OOM_EXCEEDED, `disabled`/`allow`/`proxy` network policies; documented residuals: one-second per-process CPU granularity, serialized container-wide memory updates, OOM flag staleness). Do not mark any flag `true` without implementation and test coverage.
