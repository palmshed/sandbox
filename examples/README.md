...
- `quota-aware-evaluator/`: Consumer example (not a test harness): a code evaluation system capping submissions with `cpuQuota`, written against the published package (`npm install` from the registry, then `npm start`). Shows capability discovery, timeout handling under throttling, and per-execution overrides.
- `starlark-eval/`: Consumer example (not a test harness): run Starlark programs as ordinary sandbox workloads, written against the published package (`STARLARK_BIN=/path/to/starlark npm start`). Shows interpreter provisioning, Native execution with opt-in Docker guidance, and the interpreter output and libc details a consumer would otherwise rediscover.
