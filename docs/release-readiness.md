...
additions such as new capabilities), the RFC 0007 work ships as spec and SDK
version `1.2.0`, an optional extension to the frozen 1.0.0 contract:

- `spec/version.md` reports `1.2.0`; `spec/CHANGELOG.md` carries the `[1.2.0]`
  entry; SDK `specVersion` metadata reports `1.2.0` on both backends.
- `cpuQuotaLimits` enforced by native Linux (cgroups v2, validated on real
  Linux), native Windows (Job Objects, validated on real Windows), and
  Docker (`--cpus`, validated on CI Ubuntu); macOS reports `false`.
- Tag `v1.2.0` drives `release.yml`: version gate (tag == SDK == spec),
