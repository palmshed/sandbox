...
On Windows a child that detaches from the process tree can escape accounting; Linux/macOS cover pipelines, background jobs, and chained `sh -c` children. The hard core quota (`cpuQuota`) is designed in RFC 0007 (throttle, never kill) and reported via the `cpuQuotaLimits` capability: `true` on Linux where cgroup delegation probes pass (validated green on real Linux) and on Windows where Job Object rate control probes pass (validated green on real Windows), `false` for macOS only; Docker reports `true` where the daemon API supports container updates (validated green on CI Ubuntu). Windows sampling uses `Get-CimInstance Win32_Process` (the replacement for the removed WMIC utility) and is verified in CI on `windows-latest`.
